...
typedef int node_array_t[MAX_NODES];
...
int serializeTree(node_t *tree, node_array_t tree_array, int idx){
    ...
    int right_tree_start_idx = 1 + left_tree_end_idx;
    int right_tree_end_idx = serializeTree(tree->right, tree_array, right_tree_start_idx);
    int tree_end_idx = right_tree_end_idx;

    tree_array[idx] = tree->value;
    return tree_end_idx;
}
...
node_t *deserializeTree(node_array_t tree_array, node_t *tree, int start_idx, int *end_idx){
    if (tree_array[start_idx] == -1) {
        *end_idx = start_idx;
        return NULL;
    }
    tree[start_idx].value = tree_array[start_idx];
    tree[start_idx].left = deserializeTree(tree_array, tree, start_idx + 1, end_idx);
    tree[start_idx].right = deserializeTree(tree_array, tree, *end_idx + 1, end_idx);
...
int main(void){
    ...
    printf("\n");
    root = deserializeTree(tree_array, new_tree, end_idx + 2, &end_idx);
    printf("Deserialized Tree: ");
    printTreeInOrder(root);
    printf("\n");
